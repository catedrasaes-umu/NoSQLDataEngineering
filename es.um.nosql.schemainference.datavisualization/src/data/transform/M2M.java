		...
		for (EntityVersion ev : baseEntity.getEntityversions())
		{
			...
			for (Property property : ev.getProperties())
			{
				...
				if (property instanceof Attribute)
				{
					...
					if (theType instanceof es.um.nosql.schemainference.NoSQLSchema.PrimitiveType)
					{
						if (name.equals("type") || name.equals("Type"))
						{
							type = Version_DiffFactory.eINSTANCE.createEntityType();
							((EntityType)type).setType(((es.um.nosql.schemainference.NoSQLSchema.PrimitiveType)theType).getName());
						}
						else
						{
							type = Version_DiffFactory.eINSTANCE.createPrimitiveType();
							((PrimitiveType)type).setType(((es.um.nosql.schemainference.NoSQLSchema.PrimitiveType)theType).getName());
						}
					}
					else if (theType instanceof Tuple)
					{
						...
						{
							if (tupleType instanceof es.um.nosql.schemainference.NoSQLSchema.PrimitiveType)
								((HeterogeneousTupleType)type).getType().add(((es.um.nosql.schemainference.NoSQLSchema.PrimitiveType)tupleType).getName());
