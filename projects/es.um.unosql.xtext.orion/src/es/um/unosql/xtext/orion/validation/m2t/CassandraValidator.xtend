...
class CassandraValidator
{
  ...
  def dispatch checkBasicOperation(AthenaSchema schema, EntityAddOp op)
  {
    if (op.spec.features.empty)
      errorLog.add("EntityAddOp - A new Entity should define fields: " + serializer.serialize(op))
    else
    {
      if (!op.spec.features.exists[f | f.isKey])
        errorLog.add("EntityAddOp - A new Entity should have at least one key field: " + serializer.serialize(op))
      ...
      if (op.spec.features.exists[f | f.defaultValue !== null] && op.spec.features.exists[f | f.key && f.defaultValue === null])
        errorLog.add("EntityAddOp - When defining an initial value to features, key fields need to be given an initial value too: " + serializer.serialize(op))
    }
  }
  ...
  def dispatch checkBasicOperation(AthenaSchema schema, FeatureDeleteOp op)
  {
    if (op.spec.selector.ref.contains("."))
      errorLog.add("FeatureDeleteOp - Cannot delete features with dot notation: " + serializer.serialize(op))

    val features = handler.getSimpleFeatureFromSchemaType(handler.getEntityDecl(schema, op.spec.selector.ref), op.spec.selector.target)

    if (features.empty)
      errorLog.add("FeatureDeleteOp - Cannot find a Feature to delete: " + serializer.serialize(op))
    else if (features.exists[f | f.isKey])
      errorLog.add("FeatureDeleteOp - A key attribute can't be deleted: " + serializer.serialize(op))
  }

  def dispatch checkBasicOperation(AthenaSchema schema, FeatureRenameOp op)
  {
    if (op.spec.selector.ref.contains("."))
      errorLog.add("FeatureRenameOp - Cannot rename features with dot notation: " + serializer.serialize(op))
  }

  def dispatch checkBasicOperation(AthenaSchema schema, FeatureCopyOp op)
  {
    val features = handler.getSimpleFeatureFromSchemaType(handler.getEntityDecl(schema, op.spec.targetSelector.ref), op.spec.condition.c2)
    if (features.empty || !features.exists[f | f.key])
      errorLog.add("FeatureCopyOp - Cannot copy without a key on the target condition: " + serializer.serialize(op))
  }
  ...
  def dispatch checkBasicOperation(AthenaSchema schema, FeatureMoveOp op)
  {
    val features = handler.getSimpleFeatureFromSchemaType(handler.getEntityDecl(schema, op.spec.targetSelector.ref), op.spec.condition.c2)
    if (features.empty || !features.exists[f | f.key])
      errorLog.add("FeatureMoveOp - Cannot Move without a key on the target condition: " + serializer.serialize(op))
  }

  def dispatch checkBasicOperation(AthenaSchema schema, FeatureNestOp op)
  {
    warningLog.add("FeatureNestOp - Fields will be removed and nested but cannot be populated: " + serializer.serialize(op))
  }

  def dispatch checkBasicOperation(AthenaSchema schema, FeatureUnnestOp op)
  {
    warningLog.add("FeatureUnnestOp - Fields will be unnested but will not be removed from the nest nor populated: " + serializer.serialize(op))
  }

  def dispatch checkBasicOperation(AthenaSchema schema, AttributeAddOp op)
  {
    if (op.spec.isKey)
      warningLog.add("AttributeAddOp - A new key column can't be added. Ignoring key: " + serializer.serialize(op))

    if (op.spec.selector.target.startsWith("_"))
    {
      warningLog.add("AttributeAddOp - In Cassandra a field cannot start with \"_\". Removing that character: " + serializer.serialize(op) + "\n")
    ...
    if (op.spec.defaultValue !== null)
      warningLog.add("AttributeAddOp - When adding a new column the default value is ignored: " + serializer.serialize(op) + "\n")
  }
  ...
  {
    if (op.spec.deleteId)
      warningLog.add("ReferenceMorphOp - Can't delete Id from a user-defined type. Ignoring option: " + serializer.serialize(op))
  }

  def dispatch checkBasicOperation(AthenaSchema schema, AggregateAddOp op)
  {
    if (op.spec.features.empty)
      errorLog.add("AggregateAddOp - In Cassandra an User-defined type should have fields: " + serializer.serialize(op))
    else
      for(feat : op.spec.features)
        checkSimpleDataFeature(feat)

    if (op.spec.features.exists[f | f.defaultValue !== null])
      warningLog.add("AggregateAddOp - When adding a new column the default value is ignored: " + serializer.serialize(op) + "\n")
  }
  ...
  private def checkSimpleDataFeature(SimpleDataFeature feat)
  {
    ...
    {
      warningLog.add("SimpleDataFeature - In Cassandra a field cannot start with \"_\". Removing that character: " + serializer.serialize(feat) + "\n")
  ...
  private def checkDataType(DataType theType)
  {
    if (theType === null)
    {
      ...
      switch (theType)
      {
        ...
        {
          warningLog.add("SimpleDataFeature - In Cassandra a column of type Null can't be created. Replacing by String: " + serializer.serialize(theType.eContainer) + "\n")
          return factory.createPrimitiveType("String")
        }
        case theType instanceof Map:
        {
          ...
          {
            warningLog.add("SimpleDataFeature - In Cassandra a Map has to define a primitive Key type. Replacing by String: " + serializer.serialize(theType.eContainer) + "\n")
            type.keyType = factory.createPrimitiveType("String")
          }
          if (type.valueType === null || (type.valueType instanceof Null))
          {
            warningLog.add("SimpleDataFeature - In Cassandra a Map can't be of Null values. Replacing by String: " + serializer.serialize(theType.eContainer) + "\n")
        ...
        case theType instanceof List:
        {
          ...
          {
            warningLog.add("SimpleDataFeature - In Cassandra a List can't be of Nulls. Replacing by String: " + serializer.serialize(theType.eContainer) + "\n")
        ...
        case theType instanceof Set:
        {
          ...
          if (type.elementType === null || (type.elementType instanceof Null))
          {
            warningLog.add("SimpleDataFeature - In Cassandra a Set can't be of Nulls. Replacing by String: " + serializer.serialize(theType.eContainer) + "\n")
            type.elementType = factory.createPrimitiveType("String")
          }
        ...
        case theType instanceof Tuple:
        {
          ...
          {
            warningLog.add("SimpleDataFeature - In Cassandra a Tuple can't be of Nulls. Replacing by String: " + serializer.serialize(theType.eContainer) + "\n")
            type.elements.clear
