...
class Athena2UNoSQL
{
  ...
  def uNoSQLSchema m2m(AthenaSchema schema)
  {
    ...
    val newSchema = factory.createUNoSQLSchema(nSchema.name)
  ...
  private def dispatch void insertFeatureInVariation(SimpleFeature sFeat, StructuralVariation v, boolean optional)
  {
    switch (sFeat)
    {
      ...
      case sFeat.type instanceof SimpleAggregationTarget:
      {
        val simpleAggr = sFeat.type as SimpleAggregationTarget
