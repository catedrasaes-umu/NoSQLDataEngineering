...
class AthenaFactory
{
  ...
  def CommonSpec createCommonSpec(StructureLiteral struct)
  {
    ...
    if (struct === null)
      throw new IllegalArgumentException("Create CommonSpec: Struct can't be null")
  ...
  def FeatureSet createFeatureSet(Feature... feats)
  {
    ...
    if (feats !== null && !feats.empty)
      result.features.addAll(Arrays.asList(feats))
  ...
  def StructureLiteral createStructureLiteral(FeatureSet fset)
  {
    ...
    result.spec = EcoreUtil.copy(fset)
  ...
  def SimpleFeature createSimpleFeature(String name)
  {
    ...
    if (name.equals("_id"))
      result.key = true
    // Little trick of the trade here. Let's test it
  ...
  def SimpleAggregationTarget createSimpleAggr(EObject aggrType, String multiplicity)
  {
    if (aggrType === null)
      throw new IllegalArgumentException("Create SimpleAggregation: aggrType can't be null")

    val result = factory.createSimpleAggregationTarget
  ...
  def SimpleAggregationTarget createSimpleAggr(EList<VariationDecl> aggrs, String multiplicity)
  {
    if (aggrs === null || aggrs.isEmpty)
      throw new IllegalArgumentException("Create SimpleAggregation: aggrs must contain at least one element")
