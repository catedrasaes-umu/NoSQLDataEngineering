...
class ExpressionSimplifier
{
  ...
  {
    val leftExpr = if (expr.left !== null) EcoreUtil.copy(simplify(expr.left)) else factory.createFeatureSet
    val rightExpr = if (expr.right !== null) EcoreUtil.copy(simplify(expr.right)) else factory.createFeatureSet
  ...
  {
    val result = if (expr.left !== null) EcoreUtil.copy(simplify(expr.left)) else factory.createFeatureSet
    val rightExpr = if (expr.right !== null) EcoreUtil.copy(simplify(expr.right)) else factory.createFeatureSet
      ...
      for (Feature feat1 : result.features)
      {
        ...
        else
        {
          ...
          else if (feat1 instanceof SimpleFeature && feat2 instanceof SimpleFeature)
          {
            ...
            {
              if (sFeat2.type !== null)
                sFeat1.type = EcoreUtil.copy(sFeat2.type)
