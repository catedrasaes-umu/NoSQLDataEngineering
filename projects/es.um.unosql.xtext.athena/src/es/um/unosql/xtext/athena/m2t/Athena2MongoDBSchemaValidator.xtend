...
class Athena2MongoDBSchemaValidator
{
  ...
  def void m2t(AthenaSchema schema, File outputFolder)
  {
    ...
    new CodeWriter().writeToFile(outputFolder, nSchema.name + ".js", result)
  }

  def genHeader(AthenaSchema schema)
  '''
    «schema.name» = db.getSiblingDB("«schema.name»")
  ...
  «dbName».createCollection("«entity.name»",
  {
    validator:
    {
      ...
      {
        bsonType: "object",
        «IF !handler.getFeaturesFromSchemaType(entity).empty»required: [ «FOR feat : handler.getFeaturesFromSchemaType(entity).filter(SimpleFeature).filter[f | !f.optional] SEPARATOR ", "»"«feat.name»"«ENDFOR» ],«ENDIF»
        properties:
        {
  ...
  «dbName».createCollection("«entity.name»",
  {
    ...
    {
      bsonType: "object",
      «IF !handler.getFeaturesFromSchemaType(entity).empty»required: [ «FOR feat : handler.getFeaturesFromSchemaType(entity).filter(SimpleFeature) SEPARATOR ", "»"«feat.name»"«ENDFOR» ],«ENDIF»
      properties:
      {
        «FOR feat : handler.getReducedFeaturesFromSchemaTypeAndVariations(entity) SEPARATOR ", "»
          «generateFeature(feat)»
        «ENDFOR»
      }
    }
  ...
  private def dispatch CharSequence generateType(SimpleAggregationTarget type, String name, boolean optional)
  '''
    «val aggregatedFeatures = handler.getFeaturesFromAggregation(type.aggr).filter(SimpleFeature)»
