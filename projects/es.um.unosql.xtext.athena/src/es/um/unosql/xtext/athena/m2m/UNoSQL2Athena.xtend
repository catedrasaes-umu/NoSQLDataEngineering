...
class UNoSQL2Athena
{
   ...
   *   1-Create UNoSQL EntityTypes and RelationshipTypes with its variations. No Features are created.
    ...
    else
    {
      ...
      if (ref.isFeaturedBy.empty)
      {
        ...
        val result = factory.createSimpleFeature(refName, factory.createSimpleRef(
          ...
          getCardinality(ref.lowerBound, ref.upperBound),
  ...
  private def dispatch es.um.unosql.xtext.athena.athena.Feature transformFeature(Aggregate aggr)
  {
    val result = factory.createSimpleFeature(aggr.name, factory.createSimpleAggr(
      ...
      getCardinality(aggr.lowerBound, aggr.upperBound)), aggr.isOptional)
  ...
  private def String getCardinality(int lBound, int uBound)
  {
    if (lBound == 1 && uBound == 1)
      ...
      throw new IllegalArgumentException("Error: Can't create cardinality for upper bound different of 1 and -1")
