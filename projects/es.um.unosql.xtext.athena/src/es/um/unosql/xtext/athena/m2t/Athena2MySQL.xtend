...
class Athena2MySQL
{
  ...
  {
    val nSchema = new Athena2MySQLAthena().m2m(new AthenaNormalizer().m2m(schema))
    ...
    for (EntityDecl e : nSchema.entities.sortBy[e | e.root])
      result.append(generateEntityDecl(e, handler.getReducedFeaturesFromSchemaTypeAndVariations(e)))

    new CodeWriter().writeToFile(outputFolder, nSchema.name + ".sql", result)
  }
  ...
  '''
    ...
    (
      ...
      «FOR feat : features.filter(ComposedReference)»
        FOREIGN KEY ( «feat.names.join(", ")» ) REFERENCES «feat.target.ref.name» ( «FOR feat2 : handler.getFeaturesFromSchemaType(feat.target.ref).filter(SimpleFeature).filter[f | f.key] SEPARATOR ", "»«feat2.name»«ENDFOR» ),
      «ENDFOR»
      ...
      «ENDFOR»
      «FOR feat : features.filter(SimpleFeature).filter[f | f.type instanceof SimpleAggregationTarget]»
        FOREIGN KEY ( «feat.name» ) REFERENCES «(feat.type as SimpleAggregationTarget).aggr.head instanceof EntityDecl ? ((feat.type as SimpleAggregationTarget).aggr.head as EntityDecl).name : ((feat.type as SimpleAggregationTarget).aggr.head.eContainer as EntityDecl).name» ( «getFirstSQLKeyFeature((feat.type as SimpleAggregationTarget).aggr.head instanceof EntityDecl ? (feat.type as SimpleAggregationTarget).aggr.head as EntityDecl : (feat.type as SimpleAggregationTarget).aggr.head.eContainer as EntityDecl).name» ),
      «ENDFOR»
  ...
  '''«generateType(getFirstSQLKeyFeature(type.aggr.head instanceof EntityDecl ? type.aggr.head as EntityDecl : type.aggr.head.eContainer as EntityDecl).type)»''' // The aggregation was already generated and the foreign key will be generated later on.
  ...
  {
    val result = handler.getKeyFromSchemaType(schemaType)
