package es.um.nosql.schemainference.gen.mongooseVer.main;
...
import es.um.nosql.schemainference.gen.mongooseVer.xtend.MongooseVerGenerator;
import es.um.nosql.schemainference.util.emf.ResourceManager;

public class MongooseVerGen {
	...
	{
		(new MongooseVerGen()).execute(args);
	}
	...
	protected void execute(String[] args) throws FileNotFoundException
	{
		...
		if (args.length > 1)
		{
			...
			File outFile = outputDir.toPath().resolve("mongooseVersions.js").toFile();
		...
		MongooseVerGenerator gen = new MongooseVerGenerator();
